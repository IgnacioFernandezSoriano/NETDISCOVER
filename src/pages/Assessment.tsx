...
export default function Assessment() {
  ...
  // Asegurar que phases existe y tiene elementos
  if (loading) {
    return (
  ...
  // Si no hay fases, algo va mal con la DB
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="card p-8 max-w-md w-full text-center">
            <AlertCircle size={32} className="mx-auto mb-4 text-red-500" />
            <h2 className="font-bold text-lg mb-2 text-gray-800">Database Error</h2>
            <p className="text-sm text-gray-500 mb-4">No phases found in database. Please check Supabase configuration.</p>
          </div>
        </div>
      </div>
    )
  ...
  const currentQuestions = questionsForPhase(currentPhase.id)
  ...
  const phaseTitle = getLocalizedText(currentPhase as any, 'title', lang) || currentPhase.title_en
  const phaseDesc = getLocalizedText(currentPhase as any, 'description', lang) || currentPhase.description_en
            ...
            {!saveEmailSent ? (
              ...
              <div className="p-4 rounded-xl bg-green-50 border border-green-100 mb-6 text-center">
                ...
                <p className="text-xs text-green-600">
                  {lang === 'es' ? 'Tu sesión ha sido vinculada a tu email. Puedes cerrar esta ventana.' : 'Your session has been linked to your email. You can close this window.'}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 animate-fade-in">
            ...
            <div className="space-y-4 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{t('profile.organization')}</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-2">{t('profile.type')}</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => setProfile({ ...profile, entityType: 'regulator' })}
                    className={`px-4 py-2.5 rounded-xl border-2 text-sm font-medium transition-all ${
                      profile.entityType === 'regulator'
                        ? 'border-blue-500 bg-blue-50 text-blue-700'
                        : 'border-gray-100 text-gray-500 hover:border-gray-200'
                    }`}
                  >
                    {t('profile.type.regulator')}
                  </button>
                  <button
                    onClick={() => setProfile({ ...profile, entityType: 'designated_operator' })}
                    className={`px-4 py-2.5 rounded-xl border-2 text-sm font-medium transition-all ${
                      profile.entityType === 'designated_operator'
                        ? 'border-blue-500 bg-blue-50 text-blue-700'
                        : 'border-gray-100 text-gray-500 hover:border-gray-200'
                    }`}
                  >
                    {t('profile.type.operator')}
                  </button>
                </div>
              </div>
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-3 text-sm font-semibold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all"
              >
                {t('profile.back')}
              </button>
              <button
                onClick={handleComplete}
                disabled={completing || !profile.name || !profile.organization || !profile.country}
                className="flex-2 px-10 py-3 text-sm font-semibold text-white rounded-xl transition-all hover:opacity-90 disabled:opacity-50"
                style={{ background: 'var(--brand-green)' }}
              >
                {completing ? <Loader2 size={18} className="animate-spin mx-auto" /> : t('profile.complete')}
              </button>
            </div>
          </div>
        </div>
